...
class Game:
    ...
    def play(self):
        ...
        while True:
            ...
            try:
                message=f'{self.currentPlayer.name},enter the position(1-9):'
                position=int(input(message))

                 # the update_board() method return True if
                # the user selected position is not already filled
                if self.board.update_board(position, self.currentPlayer.type):

                    self.board.print_board()
                    
                    # checking winner each time after updating the board
                    if self.board.check_winner(self.currentPlayer.type):
                        print(self.currentPlayer.name,'wins !')
                        break
                      # Check for draw after the board has been updated
